...
def pemasukanBulanan(cursor):
# Total Data Pemasukan Bulan Lalu
    ...
    query_last_month = f"SELECT SUM(jumlah_pemasukan) FROM pemasukan WHERE tanggal_pemasukan BETWEEN '{last_month_start}' AND '{last_month_end}'"
    ...
    # Total Data Pemasukan Bulan Ini
    this_month_start = datetime.now().replace(day=1).strftime('%Y-%m-%d')
    this_month_end = datetime.now().strftime('%Y-%m-%d')
    query_this_month = f"SELECT SUM(jumlah_pemasukan) FROM pemasukan WHERE tanggal_pemasukan BETWEEN '{this_month_start}' AND '{this_month_end}'"
